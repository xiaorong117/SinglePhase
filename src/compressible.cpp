...
void PNMsolver::para_cal(double mode)
{
	...
	double end = omp_get_wtime();
	printf("para_cal start = %.16g\tend = %.16g\tdiff = %.16g\n",
		   start, end, end - start);
}
...
void PNMsolver::Matrix_permeability(double mode)
{
	...
	double end = omp_get_wtime();
	printf("matrix start = %.16g\tend = %.16g\tdiff = %.16g\n",
		   start, end, end - start);
};
...
int main(int argc, char **argv)
{
	...
	// double fanshu2{0};
	// auto R = 30e-9;
	// ofstream Kom_hp("Kom_hp.txt", std::ios::app);
	// ofstream Kom_hp_out("Kom_hp_out.txt");
	// for (size_t j = 0; j < 100; j++)
	// {
	// 	R += 0.1e-9;
	// 	dx = 0;
	// 	fanshu2 = 0;
	// 	ifstream simu_hp("apparent_Permeability_amgx_hp.txt");
	// 	for (size_t i = 1; i < 51; i++)
	// 	{
	// 		auto pressure = i * 1e6;
	// 		auto z{Berea.compre(pressure)};
	...
	// 		double beta = 4;
	// 		double Slip = (1 + alpha * Knusen_number) * (1 + beta * Knusen_number / (1 + Knusen_number));
	// 		auto out = Slip * 1.09562e-18 + 1 / rho * vis * Ds * n_max_ad * K_langmuir / pow(1 + K_langmuir * pressure, 2);
	// 		out = out / 1e-21;
	// 		// Kom_hp_out << out << endl;
	// 		simu_hp >> si;
	// 		fanshu2 += pow(si, 2);
	// 		dx += pow(out - si, 2);
	// 	}
	// 	Kom_hp << "R = " << sqrt(dx) / sqrt(fanshu2) << " length = " << R << " fanshu2 = " << sqrt(fanshu2) << endl;
	// 	simu_hp.close();
	// }
	// Kom_hp.close();
	...
	// double dx_lp{0};
	// R = 2e-9;
	// ofstream Kom_lp("Kom_lp.txt", std::ios::app);
	// ofstream Kom_lp_out("Kom_lp_out.txt");
	// for (size_t j = 0; j < 100; j++)
	// {
	// 	R += 0.1e-9;
	// 	dx_lp = 0;
	// 	fanshu2 = 0;
	// 	ifstream simu_lp("apparent_Permeability_amgx_lp.txt");
	// 	for (size_t i = 1; i < 51; i++)
	// 	{
	// 		auto pressure = i * 1e6;
	// 		auto z{Berea.compre(pressure)};
	...
	// 		double beta = 4;
	// 		double Slip = (1 + alpha * Knusen_number) * (1 + beta * Knusen_number / (1 + Knusen_number));
	// 		auto out = Slip * 1.48106e-20 + 1 / rho * vis * Ds * n_max_ad * K_langmuir / pow(1 + K_langmuir * pressure, 2);
	// 		out = out / 1e-21;
	// 		// Kom_lp_out << out << endl;
	// 		simu_lp >> si_lp;
	// 		fanshu2 += pow(si_lp, 2);
	// 		dx_lp += pow(out - si_lp, 2);
	// 	}
	// 	Kom_lp << "Residual = " << sqrt(dx_lp) / sqrt(fanshu2) << " length = " << R << " fanshu2 = " << sqrt(fanshu2) << endl;
	// 	simu_lp.close();
	// }
	// Kom_lp.close();
	...
	// 	auto out = Slip * 1.09562e-18 + 1 / rho * vis * Ds * n_max_ad * K_langmuir / pow(1 + K_langmuir * pressure, 2);
	// 	out = out / 1e-21;
	// 	Kom_hp_out << out << " " << pressure / 1e6 << endl;
	// }
	// Kom_hp_out.close();
	...
	// 	auto out = Slip * 1.48106e-20 + 1 / rho * vis * Ds * n_max_ad * K_langmuir / pow(1 + K_langmuir * pressure, 2);
	// 	out = out / 1e-21;
	// 	Kom_lp_out << out << " " << pressure / 1e6 << endl;
	// }
	// Kom_lp_out.close();
