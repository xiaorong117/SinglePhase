...
{
	ifstream porefile("size-4-4-Pct-2-01_Dry-digital rock-6.5um-porosity-0.239-400-400-400-pore-Z.txt", ios::in);  //ios::in-以输入方式打开文件
	if (!porefile.is_open())
	{
	...
	for (int i = 0; i < pn; i++)
	{
		porefile >> Pb[i].X >> Pb[i].Y >> Pb[i].Z >> Pb[i].Radiu >> Pb[i].Half_coord >> Pb[i].Accumulative_half >> Pb[i].type >> Pb[i].porosity >> Pb[i].micropore_radiu;
	}
	porefile.close();
	for (int i = 0; i < pn; i++)
	{
		...
		Pb[i].micropore_radiu = 1e-6 * Pb[i].micropore_radiu;
	}

	ifstream throatfile("size-4-4-Pct-2-01_Dry-digital rock-6.5um-porosity-0.239-400-400-400-throat-Z.txt", ios::in);
...
void PNMsolver::para_cal()
{
	...
	{                     
		...
		else if (Pb[Tb[i].ID_1].type == 0 && Pb[Tb[i].ID_2].type == 1)
		{
			...
			k2 = Pb[Tb[i].ID_2].porosity * pow(Pb[Tb[i].ID_2].micropore_radiu, 2) / (32 * tortuosity * tortuosity);
		...
		else
		{
			...
			k1 = Pb[Tb[i].ID_1].porosity * pow(Pb[Tb[i].ID_1].micropore_radiu, 2) / (32 * tortuosity * tortuosity);
			k2 = Pb[Tb[i].ID_2].porosity * pow(Pb[Tb[i].ID_2].micropore_radiu, 2) / (32 * tortuosity * tortuosity);
