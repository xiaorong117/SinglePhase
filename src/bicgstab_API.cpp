                 ...
                 double *solution)
{
    ...
    CHECK_CUDA(cudaMemcpy(solution, d_X.ptr, m * sizeof(double), cudaMemcpyDeviceToHost));
    //--------------------------------------------------------------------------
...

{
    ...
    for (int i = 1; i <= maxIterations; i++)
    {
        ...
        // ### 10 ### alpha = (R'0, R_i-1) / (R'0, A * P_aux)
        //    (a) V = A * P_aux
        CHECK_CUDA(cudaMemcpy(d_P_aux.ptr, d_P.ptr, m * sizeof(double),
                              cudaMemcpyDeviceToDevice))
        CHECK_CUSPARSE(cusparseSpMV(cusparseHandle,
                                    CUSPARSE_OPERATION_NON_TRANSPOSE, &one,
                                    matA, d_P_aux.vec, &zero, d_V.vec,
        ...
        //    (a) M_L^-1 S => tmp    (triangular solver)
        cudaMemset(d_tmp.ptr, 0x0, m * sizeof(double));
        CHECK_CUDA(cudaMemcpy(d_S_aux.ptr, d_S.ptr, m * sizeof(double),
                              cudaMemcpyDeviceToDevice))
        //----------------------------------------------------------------------
        // ### 15 ### omega = (A * S_aux, s) / (A * S_aux, A * S_aux)
        //    (a) T = A * S_aux
...
int API_BICGSTAB(int maxIter, double toler, int *rows, int *columns, double *values, int n, double *rhs)
{
    const int maxIterations = maxIter;
    const double tolerance = toler;

    int base = 0;
    int m = n;
    int *h_A_rows = rows;
    int *h_A_columns = columns;
    double *h_A_values = values;
    int num_offsets = m + 1;
    int nnz = h_A_rows[m];
    ...
    CHECK_CUDA(cudaMemcpy(d_M_values, h_A_values, nnz * sizeof(double),
                          cudaMemcpyHostToDevice))
    CHECK_CUDA(cudaMemcpy(d_B.ptr, rhs, m * sizeof(double),
                          cudaMemcpyHostToDevice))
    //--------------------------------------------------------------------------
    ...
    gpu_BiCGStab(cublasHandle, cusparseHandle, m,
                 ...
                 d_tmp, d_bufferMV, maxIterations, tolerance, rhs);
    //--------------------------------------------------------------------------
    // ### Free resources ###
...
int API_UNPBICGSTAB(int maxIter, double toler, int *rows, int *columns, double *values, int n, double *rhs)
{
    ...
    // ### Run BiCGStab computation ###
    printf("BiCGStab loop:\n");
    gpu_UNPBiCGStab(cublasHandle, cusparseHandle, m,
                    matA,
                    d_B, d_X, d_R0, d_R, d_P, d_P_aux, d_S, d_S_aux, d_V, d_T,
                    d_tmp, d_bufferMV, maxIterations, tolerance, rhs);
    //--------------------------------------------------------------------------
