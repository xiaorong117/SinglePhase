...
std::vector<int> inlet_boundary(512,0);
...
namespace Fluid_property
{
	...
	namespace kong
	{
		double D_dispersion_macro{0.85e-9 * 237.885 / 100}; // 237.885 / 100 for 1st,  233.65 / 100 for 2nd, 396.055 / 100 for 3rd.
	...
	for (int j = Pb[Pore_id].full_accum_ori - Pb[Pore_id].full_coord_ori; j < Pb[Pore_id].full_accum_ori; j++)
	{
		...
		if (Tb[j].ID_2 < inlet) // 大孔进口
		{
			...
			if (exists)
			{
				if (Pb[Tb[j].ID_1].pressure > Pb[Tb[j].ID_2].pressure)
				{
					RETURN += 2.15e-4 * pi * pow(Tb[j].Radiu,2);
				}
				else
				{
					RETURN += 2.15e-4 * pi * pow(Tb[j].Radiu,2);
			...
			else
			{
				if (Pb[Tb[j].ID_1].pressure > Pb[Tb[j].ID_2].pressure)
				{
					RETURN += 1.733e-4 * pi * pow(Tb[j].Radiu,2);
				}
				else
				{
					RETURN += 1.733e-4 * pi * pow(Tb[j].Radiu,2);
		...
		if (Tb[j].ID_2 < inlet) // 大孔进口
		{
			...
			if (Pb[Tb[j].ID_1].pressure > Pb[Tb[j].ID_2].pressure)
			{
				...
				if (exists)
				{
					...
					Return += Wi * 2.15e-4 * pi * pow(Tb[j].Radiu,2); // co2 advection term
				}
				else
				{
					Return += Conductivity_dis_co2 * (Wi - Wjs[iCounter]);
					Return += Wi * 1.733e-4 * pi * pow(Tb[j].Radiu,2); // co2 advection term
				}
			}
			else
			{
				...
				if (exists)
				{
					Return += Wjs[iCounter] * 2.15e-4 * (Pi - Pjs[iCounter]); // co2 advection term
					Return += Conductivity_dis_co2 * (Wi - Wjs[iCounter]);
				}
				else
				{
					Return += Wjs[iCounter] * 1.733e-4 * (Pi - Pjs[iCounter]); // co2 advection term
					Return += Conductivity_dis_co2 * (Wi - Wjs[iCounter]);
				}
			}
		}
