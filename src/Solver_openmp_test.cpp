...
#pragma omp parallel for num_threads(int(OMP_PARA))
	...
	for (int i = 0; i < 2 * tn; i++)
	{
		...
		// // if (Tb_in[i].ID_1 < macro_n && Tb_in[i].ID_2 < macro_n)
		// // {
		// // 	Knusen_number = Average_visco / Average_pressure * sqrt(pi * Average_compre * 8.314 * Temperature / (2 * 0.016)) / (Pb[Tb_in[i].ID_1].Radiu + Pb[Tb_in[i].ID_2].Radiu);
		// // }
		// if (Tb_in[i].ID_1 < macro_n && Tb_in[i].ID_2 >= macro_n)
		// {
		// 	Knusen_number = Average_visco / Average_pressure * sqrt(pi * Average_compre * 8.314 * Temperature / (2 * 0.016)) / (Pb[Tb_in[i].ID_1].Radiu + Pb[Tb_in[i].ID_2].radius_micro);
		// }
		// if (Tb_in[i].ID_1 >= macro_n && Tb_in[i].ID_2 < macro_n)
		// {
		// 	Knusen_number = Average_visco / Average_pressure * sqrt(pi * Average_compre * 8.314 * Temperature / (2 * 0.016)) / (Pb[Tb_in[i].ID_1].radius_micro + Pb[Tb_in[i].ID_2].Radiu);
		// }
		// else
		// {
		...
		// 计算滑移项
		/*按照论文中的传导系数计算方式编程*/
		double Pressure1{Pb[Tb_in[i].ID_1].pressure + refer_pressure}; 
		double Pressure2{Pb[Tb_in[i].ID_2].pressure + refer_pressure};
		double z1{Pb[Tb_in[i].ID_1].compre};
		double z2{Pb[Tb_in[i].ID_2].compre};
		double viscosity1{Pb[Tb_in[i].ID_1].visco};
		double viscosity2{Pb[Tb_in[i].ID_2].visco};
		double Knusen_number1{0};
		double Knusen_number2{0};

		if (Tb_in[i].ID_1 < macro_n && Tb_in[i].ID_2 < macro_n)
		{
			Knusen_number1 = viscosity1 / Pressure1 * sqrt(pi * z1 * 8.314 * Temperature / (2 * 0.016)) / (Tb_in[i].Radiu * 2);
			Knusen_number2 = viscosity2 / Pressure2 * sqrt(pi * z2 * 8.314 * Temperature / (2 * 0.016)) / (Tb_in[i].Radiu * 2);
		}
		if (Tb_in[i].ID_1 < macro_n && Tb_in[i].ID_2 >= macro_n)
		{
			Knusen_number1 = viscosity1 / Pressure1 * sqrt(pi * z1 * 8.314 * Temperature / (2 * 0.016)) / (Pb[Tb_in[i].ID_1].Radiu * 2);
			Knusen_number2 = viscosity2 / Pressure2 * sqrt(pi * z2 * 8.314 * Temperature / (2 * 0.016)) / (Pb[Tb_in[i].ID_2].radius_micro * 2);
		}
		if (Tb_in[i].ID_1 >= macro_n && Tb_in[i].ID_2 < macro_n)
		{
			Knusen_number1 = viscosity1 / Pressure1 * sqrt(pi * z1 * 8.314 * Temperature / (2 * 0.016)) / (Pb[Tb_in[i].ID_1].radius_micro * 2);
			Knusen_number2 = viscosity2 / Pressure2 * sqrt(pi * z2 * 8.314 * Temperature / (2 * 0.016)) / (Pb[Tb_in[i].ID_2].Radiu * 2);
		}
		else
		{
			Knusen_number1 = viscosity1 / Pressure1 * sqrt(pi * z1 * 8.314 * Temperature / (2 * 0.016)) / (Pb[Tb_in[i].ID_1].radius_micro * 2);
		...
		double alpha1 = 1.358 * 2 / pi * atan(4 * pow(Knusen_number1, 0.4));
		double alpha2 = 1.358 * 2 / pi * atan(4 * pow(Knusen_number2, 0.4));
		double beta = 4;
		double Slip1{(1 + alpha1 * Knusen_number1) * (1 + beta * Knusen_number1 / (1 + Knusen_number1))};
		double Slip2{(1 + alpha2 * Knusen_number2) * (1 + beta * Knusen_number2 / (1 + Knusen_number2))};
		...
		{
			temp1 = Slip1 * pi * pow(Pb[Tb_in[i].ID_1].Radiu, 3) / (8 * viscosity1);
			...
			temp2 = abs(Slip2 * Pb[Tb_in[i].ID_2].km * Tb_in[i].Radiu * angle2 / (viscosity2 * length2));
		...
		{
			temp2 = Slip2 * pi * pow(Pb[Tb_in[i].ID_2].Radiu, 3) / (8 * viscosity2);
			...
			temp1 = abs(Slip1 * Pb[Tb_in[i].ID_1].km * Tb_in[i].Radiu * angle1 / (viscosity1 * length1));
		...
		{
			temp1 = abs(Slip1 * Pb[Tb_in[i].ID_1].km * Tb_in[i].Radiu / (viscosity1 * Tb_in[i].Length/2));
			temp2 = abs(Slip2 * Pb[Tb_in[i].ID_2].km * Tb_in[i].Radiu / (viscosity2 * Tb_in[i].Length/2));
			temp11 = abs(Tb_in[i].Radiu * Ds * n_max_ad / Tb_in[i].Length/2);
			temp22 = abs(Tb_in[i].Radiu * Ds * n_max_ad / Tb_in[i].Length/2);
			Tb_in[i].Conductivity = temp1 * temp2 / (temp1 + temp2);
			Tb_in[i].Surface_diff_conduc = temp11 * temp22 / (temp11 + temp22);
		}
		else
...
// #endif
	for (int i = 0; i < 2 * tn; i++)
	{
		...
		{
			temp1 = abs(Pb[Tb_in[i].ID_1].km * Tb_in[i].Radiu / (Pb[Tb_in[i].ID_1].visco * Tb_in[i].Length/2));
			temp2 = abs(Pb[Tb_in[i].ID_2].km * Tb_in[i].Radiu / (Pb[Tb_in[i].ID_2].visco * Tb_in[i].Length/2));
			temp11 = abs(Tb_in[i].Radiu * Ds * n_max_ad / Tb_in[i].Length/2);
			temp22 = abs(Tb_in[i].Radiu * Ds * n_max_ad / Tb_in[i].Length/2);
			Tb_in[i].Conductivity = temp1 * temp2 / (temp1 + temp2);
			Tb_in[i].Surface_diff_conduc = temp11 * temp22 / (temp11 + temp22);
