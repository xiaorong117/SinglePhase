...
double voxel_size = 2e-6;				   // 像素尺寸，单位m
int outlet_element_n[3] = {500, 500, 500}; // 模型的在各个方向的像素数量，本模拟所用模型为正方形
double time_factor = 10;				   // 时间因子，确定计算的时间步
double tortuosity = 1.75;
double aver_micro_radius = 1.81e-6;

const int pn = 15974;									// 孔数量
const int tn = 91656;									// 吼道数量
const int inlet = 508, outlet = 415, LP = 11580, MP = 3471;	// 进口边界单元，出口边界单元，分割后大孔，小孔数量
const int NA = (tn - inlet - outlet) * 2 + LP + MP; // 求解孔喉矩阵中非零的量的个数
// const int inlet = 1, outlet = 1, LP = 1, MP = 1;	// 进口边界单元，出口边界单元，分割后大孔，小孔数量
// const int NA = (tn - inlet - outlet) * 2 + LP + MP; // 求解孔喉矩阵中非零的量的个数
...
{
	ifstream porefile("500-500-500-pore-Z.txt", ios::in); // ios::in-以输入方式打开文件
	if (!porefile.is_open())
	{
	...
	for (int i = 0; i < pn; i++)
	{
		porefile >> Pb[i].X >> Pb[i].Y >> Pb[i].Z >> Pb[i].Radiu >> Pb[i].Half_coord >> Pb[i].Accumulative_half >> Pb[i].type;
	}
	porefile.close();
	for (int i = 0; i < pn; i++)
	{
	...
	ifstream throatfile("500-500-500-throat-Z.txt", ios::in);
...
void PNMsolver::PressureMatrix()
{
	...
	{
		a_debug << A[i][0] << " " << A[i][1] << " " << A[i][2] << endl;
	}
	a_debug.close();

	ofstream B_debug("B_debug.txt");
	for (size_t i = 0; i < LP + MP; i++)
	{
		B_debug << B[i] << endl;
	}
	B_debug.close();
