...
int conjugateGradient_solver(int iters_, double tol_) {
  ...
  double rz_new, rz_old, rr, r1, dot_pAp;

  double *x;
  double *rhs;
  double *d_x;
  double *d_r, *d_p, *d_Ax, *d_z, *d_zm1, *d_zm2; // 添加中间变量
  int k;
  double alpha, beta;
  ...
  void *sv_buffer_L = NULL;
  void *sv_buffer_U = NULL;
  ...
  checkCudaErrors(
      cudaMalloc((void **)&d_zm1, N * sizeof(double))); // L求解中间变量
  checkCudaErrors(
      cudaMalloc((void **)&d_zm2, N * sizeof(double))); // U求解中间变量
  ...
  // ================= ILU预条件子计算 =================
  cout << "Computing ILU0 preconditioner..." << endl;

  // 创建ILU结构
  checkCudaErrors(cusparseCreateCsrilu02Info(&info_M));

  // 计算ILU分解所需缓冲区大小
  int bufferSize_M;
  checkCudaErrors(cusparseDcsrilu02_bufferSize(
      cusparseHandle, N, nnz, descr_M, d_csr_values, d_csr_offsets,
      d_csr_columns, info_M, &bufferSize_M));

  // 分配缓冲区
  checkCudaErrors(cudaMalloc(&ilu_buffer, bufferSize_M));

  // 分析结构
  checkCudaErrors(cusparseDcsrilu02_analysis(
      cusparseHandle, N, nnz, descr_M, d_csr_values, d_csr_offsets,
      d_csr_columns, info_M, CUSPARSE_SOLVE_POLICY_USE_LEVEL, ilu_buffer));

  // 检查结构零元素
  cudaError_t err = cudaGetLastError();
  if (err != cudaSuccess) {
    checkCudaErrors(
        cusparseXcsrilu02_zeroPivot(cusparseHandle, info_M, &structural_zero));
  ...
  // 检查数值零元素
  checkCudaErrors(
      cusparseXcsrilu02_zeroPivot(cusparseHandle, info_M, &numerical_zero));
  if (numerical_zero != -1) {
  ...
  // 创建三角求解信息结构
  checkCudaErrors(cusparseCreateCsrsv2Info(&info_L));
  checkCudaErrors(cusparseCreateCsrsv2Info(&info_U));

  // 计算L求解缓冲区大小
  int bufferSize_L;
  checkCudaErrors(cusparseDcsrsv2_bufferSize(
      cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE, N, nnz, descr_M,
      d_csr_values, d_csr_offsets, d_csr_columns, info_L, &bufferSize_L));

  // 计算U求解缓冲区大小
  int bufferSize_U;
  checkCudaErrors(cusparseDcsrsv2_bufferSize(
      cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE, N, nnz, descr_M,
      d_csr_values, d_csr_offsets, d_csr_columns, info_U, &bufferSize_U));

  // 分配L求解缓冲区
  checkCudaErrors(cudaMalloc(&sv_buffer_L, bufferSize_L));

  // 分析L求解结构
  checkCudaErrors(cusparseDcsrsv2_analysis(
      cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE, N, nnz, descr_M,
      d_csr_values, d_csr_offsets, d_csr_columns, info_L,
      CUSPARSE_SOLVE_POLICY_USE_LEVEL, sv_buffer_L));

  // 分配U求解缓冲区
  checkCudaErrors(cudaMalloc(&sv_buffer_U, bufferSize_U));

  // 分析U求解结构
  checkCudaErrors(cusparseDcsrsv2_analysis(
      cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE, N, nnz, descr_M,
      d_csr_values, d_csr_offsets, d_csr_columns, info_U,
      CUSPARSE_SOLVE_POLICY_USE_LEVEL, sv_buffer_U));

  cout << "ILU0 preconditioner computation complete." << endl;
  ...
  // 应用ILU预条件子: z = M^{-1} r
  cout << "Applying ILU preconditioner..." << endl;

  // L求解: L * zm1 = r
  checkCudaErrors(cusparseDcsrsv2_solve(
      cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE, N, nnz, &one, descr_M,
      d_csr_values, d_csr_offsets, d_csr_columns, info_L, d_r, d_zm1,
      CUSPARSE_SOLVE_POLICY_USE_LEVEL, sv_buffer_L));

  // U求解: U * z = zm1
  checkCudaErrors(cusparseDcsrsv2_solve(
      cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE, N, nnz, &one, descr_M,
      d_csr_values, d_csr_offsets, d_csr_columns, info_U, d_zm1, d_z,
      CUSPARSE_SOLVE_POLICY_USE_LEVEL, sv_buffer_U));
  ...
  if (ilu_buffer)
    cudaFree(ilu_buffer);
  if (sv_buffer_L)
    cudaFree(sv_buffer_L);
  if (sv_buffer_U)
    cudaFree(sv_buffer_U);
