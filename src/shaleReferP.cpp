...
int Time_step{0};
int percentage_production_counter{0};
double pyhsic_time{0};
double machine_time{0};
// int Time_step{43};
// int percentage_production_counter{43};
// double pyhsic_time{1e-09};
// double machine_time{3302};

// int Time_step{420};
// int percentage_production_counter{45};
// double pyhsic_time{9.9791e-05};
// double machine_time{12183};
int Flag_eigen{true};
int flag = 2;
int flag1 = 4;
...
public:
	...
	void Paramentinput(double i);  // 读取单重孔网文件
	void Para_cal_REV();		   //
	...
	void AMGXsolver(); // 产气
	void AMGXsplver_REV();
	void AMGXsolver_apparent_permeability_REV();
	void PressureMatrix_REV();
	void PressureMatrix_Per_REV();
...
double PNMsolver::clay_loss_per_step()
{
	...
	for (int i = inlet; i < macro_n - outlet; i++)
	{
		if (Pb[i].type == 0)
		{
			clay_loss_per_step += (Pb[i].pressure_old + refer_P) * Pb[i].volume * (porosity_clay1 + (1 - porosity_clay2) * micro_porosity_Clay) * 16 / (Pb[i].compre_old * 8.314 * Temperature) - (Pb[i].pressure + refer_P) * Pb[i].volume * (porosity_clay1 + (1 - porosity_clay2) * micro_porosity_Clay) * 16 / (Pb[i].compre * 8.314 * Temperature);
		}
	}
...
double PNMsolver::fracture_loss_per_step()
{
	...
	for (int i = inlet; i < macro_n - outlet; i++)
	{
		if (Pb[i].type == 1)
		{
			fracture_loss_per_step += (Pb[i].pressure_old + refer_P) * Pb[i].volume * 16 / (Pb[i].compre_old * 8.314 * Temperature) - (Pb[i].pressure + refer_P) * Pb[i].volume * 16 / (Pb[i].compre * 8.314 * Temperature);
		}
	}
...
double PNMsolver::OM_HP_ad_loss_per_step()
{
	...
	for (int i = inlet; i < macro_n - outlet; i++)
	{
		double compre_1 = compre(Pb[i].pressure_old + refer_P);
		double compre_2 = compre(Pb[i].pressure + refer_P);
		double n_ad1 = n_max_ad * (K_langmuir * (Pb[i].pressure_old + refer_P) / (1 + K_langmuir * (Pb[i].pressure_old + refer_P)));
		double n_ad2 = n_max_ad * (K_langmuir * (Pb[i].pressure + refer_P) / (1 + K_langmuir * (Pb[i].pressure + refer_P)));
		if (Pb[i].type == 1)
		{
...
double PNMsolver::OM_HP_free_loss_per_step()
{
	...
	for (int i = inlet; i < macro_n - outlet; i++)
	{
		...
		if (Pb[i].type == 1)
		{
			OM_HP_free_loss_per_step += (Pb[i].pressure_old + refer_P) * ((1 - porosity_HP2) * (micro_porosity_HP - n_ad1 / Rho_ad) + porosity_HP1) * Pb[i].volume * 16 / (compre_1 * 8.314 * Temperature) - (Pb[i].pressure + refer_P) * Pb[i].volume * ((1 - porosity_HP2) * (micro_porosity_HP - n_ad2 / Rho_ad) + porosity_HP1) * 16 / (compre_2 * 8.314 * Temperature);
		}
	}
...
double PNMsolver::OM_LP_free_loss_per_step()
{
	...
	for (int i = inlet; i < macro_n - outlet; i++)
	{
		...
		if (Pb[i].type == 1)
		{
			OM_LP_free_loss_per_step += (Pb[i].pressure_old + refer_P) * ((1 - porosity_LP2) * (micro_porosity_LP - n_ad1 / Rho_ad) + porosity_LP1) * Pb[i].volume * 16 / (compre_1 * 8.314 * Temperature) - (Pb[i].pressure + refer_P) * Pb[i].volume * ((1 - porosity_LP2) * (micro_porosity_LP - n_ad2 / Rho_ad) + porosity_LP1) * 16 / (compre_2 * 8.314 * Temperature);
		}
	}
...
double PNMsolver::REV_OUT()
{
	...
	for (int i = macro_n - outlet; i < macro_n; i++)
	{
		for (int j = Pb[i].full_accum - Pb[i].full_coord; j < Pb[i].full_accum; j++)
		{
			...
			{
				rho = (Pb[Tb[j].ID_1].pressure + refer_P) / (Pb[Tb[j].ID_1].compre * 8.314 * Temperature); // rho mol/m^3
			}
			else
			{
				rho = (Pb[Tb[j].ID_2].pressure + refer_P) / (Pb[Tb[j].ID_2].compre * 8.314 * Temperature); // rho mol/m^3
			}
...
void PNMsolver::Para_cal_REV()
{
	...
	for (int i = 0; i < pn; i++)
	{
		...
		Pb[i].compre = compre(Pb[i].pressure + refer_P);
		Pb[i].compre_old = Pb[i].compre;
		Pb[i].visco = visco(Pb[i].pressure + refer_P, Pb[i].compre, Temperature);
	...
	for (int i = inlet; i < macro_n - outlet; i++)
	{
		double compre_1 = compre(inlet_pre + refer_P);
		double compre_2 = compre(outlet_pre + refer_P);

		double n_ad1 = n_max_ad * (K_langmuir * (inlet_pre + refer_P) / (1 + K_langmuir * (inlet_pre + refer_P)));
		double n_ad2 = n_max_ad * (K_langmuir * (outlet_pre + refer_P) / (1 + K_langmuir * (outlet_pre + refer_P)));
		if (Pb[i].type == 0)
		{
			total_clay += (inlet_pre + refer_P) * Pb[i].volume * (porosity_clay1 + (1 - porosity_clay2) * micro_porosity_Clay) * 16 / (compre_1 * 8.314 * Temperature) - (outlet_pre + refer_P) * Pb[i].volume * (porosity_clay1 + (1 - porosity_clay2) * micro_porosity_Clay) * 16 / (compre_2 * 8.314 * Temperature);
		}
		else if (Pb[i].type == 1)
		{
			total_fracture += (inlet_pre + refer_P) * Pb[i].volume * 16 / (compre_1 * 8.314 * Temperature) - (outlet_pre + refer_P) * Pb[i].volume * 16 / (compre_2 * 8.314 * Temperature);
		}
		else if (Pb[i].type == 2)
		{
			total_OM_HP_free += (inlet_pre + refer_P) * ((1 - porosity_HP2) * (micro_porosity_HP - n_ad1 / Rho_ad) + porosity_HP1) * Pb[i].volume * 16 / (compre_1 * 8.314 * Temperature) - (outlet_pre + refer_P) * Pb[i].volume * ((1 - porosity_HP2) * (micro_porosity_HP - n_ad2 / Rho_ad) + porosity_HP1) * 16 / (compre_2 * 8.314 * Temperature);
			total_OM_HP_ad += Pb[i].volume * (1 - porosity_HP2) * (n_ad1 - n_ad2) * 1000;
		}
		else if (Pb[i].type == 3)
		{
			total_OM_LP_free += (inlet_pre + refer_P) * ((1 - porosity_LP2) * (micro_porosity_LP - n_ad1 / Rho_ad) + porosity_LP1) * Pb[i].volume * 16 / (compre_1 * 8.314 * Temperature) - (outlet_pre + refer_P) * Pb[i].volume * ((1 - porosity_LP2) * (micro_porosity_LP - n_ad2 / Rho_ad) + porosity_LP1) * 16 / (compre_2 * 8.314 * Temperature);
	...
	for (int i = 0; i < 2 * tn; i++)
	{
		...
		Rho_ID1 = 0.016 * (Pb[Tb_in[i].ID_1].pressure + refer_P) / (Pb[Tb_in[i].ID_1].compre * 8.314 * Temperature);
		Rho_ID2 = 0.016 * (Pb[Tb_in[i].ID_2].pressure + refer_P) / (Pb[Tb_in[i].ID_2].compre * 8.314 * Temperature);
		if (Pb[Tb_in[i].ID_1].type == 0) // clay
		{
			Knusen_number_ID1 = Pb[Tb_in[i].ID_1].visco / (Pb[Tb_in[i].ID_1].pressure + refer_P) * sqrt(pi * Pb[Tb_in[i].ID_1].compre * 8.314 * Temperature / (2 * 0.016)) / (L_CLAY);
		...
		{
			Knusen_number_ID1 = Pb[Tb_in[i].ID_1].visco / (Pb[Tb_in[i].ID_1].pressure + refer_P) * sqrt(pi * Pb[Tb_in[i].ID_1].compre * 8.314 * Temperature / (2 * 0.016)) / (L_OM_1);
			Slip_ID1 = Function_Slip(Knusen_number_ID1);
			Apparent_K_ID1 = Slip_ID1 * K_OM_HP + Pb[Tb_in[i].ID_1].visco * Ds * n_max_ad * K_langmuir / pow(1 + K_langmuir * (Pb[Tb_in[i].ID_1].pressure + refer_P), 2) / Rho_ID1;
		}
		else if (Pb[Tb_in[i].ID_1].type == 3) // OM_type2
		{
			Knusen_number_ID1 = Pb[Tb_in[i].ID_1].visco / (Pb[Tb_in[i].ID_1].pressure + refer_P) * sqrt(pi * Pb[Tb_in[i].ID_1].compre * 8.314 * Temperature / (2 * 0.016)) / (L_OM_2);
			Slip_ID1 = Function_Slip(Knusen_number_ID1);
			Apparent_K_ID1 = Slip_ID1 * K_OM_LP + Pb[Tb_in[i].ID_1].visco * Ds * n_max_ad * K_langmuir / pow(1 + K_langmuir * (Pb[Tb_in[i].ID_1].pressure + refer_P), 2) / Rho_ID1;
		}
		// 6.96e-9
		if (Pb[Tb_in[i].ID_2].type == 0) // clay
		{
			Knusen_number_ID2 = Pb[Tb_in[i].ID_2].visco / (Pb[Tb_in[i].ID_2].pressure + refer_P) * sqrt(pi * Pb[Tb_in[i].ID_2].compre * 8.314 * Temperature / (2 * 0.016)) / (L_CLAY);
		...
		{
			Knusen_number_ID2 = Pb[Tb_in[i].ID_2].visco / (Pb[Tb_in[i].ID_2].pressure + refer_P) * sqrt(pi * Pb[Tb_in[i].ID_2].compre * 8.314 * Temperature / (2 * 0.016)) / (L_OM_1);
			Slip_ID2 = Function_Slip(Knusen_number_ID2);
			Apparent_K_ID2 = Slip_ID2 * K_OM_HP + Pb[Tb_in[i].ID_2].visco * Ds * n_max_ad * K_langmuir / pow(1 + K_langmuir * (Pb[Tb_in[i].ID_2].pressure + refer_P), 2) / Rho_ID2;
		}
		else if (Pb[Tb_in[i].ID_2].type == 3) // OM_type2
		{
			Knusen_number_ID2 = Pb[Tb_in[i].ID_2].visco / (Pb[Tb_in[i].ID_2].pressure + refer_P) * sqrt(pi * Pb[Tb_in[i].ID_2].compre * 8.314 * Temperature / (2 * 0.016)) / (L_OM_2);
			Slip_ID2 = Function_Slip(Knusen_number_ID2);
			Apparent_K_ID2 = Slip_ID2 * K_OM_LP + Pb[Tb_in[i].ID_2].visco * Ds * n_max_ad * K_langmuir / pow(1 + K_langmuir * (Pb[Tb_in[i].ID_2].pressure + refer_P), 2) / Rho_ID2;
		}
...
void PNMsolver::AMGXsplver_REV()
{
	...
	do
	{
		...
		for (int i = inlet; i < macro_n - outlet; i++)
		{
			...
			if (Pb[i].type == 0)
			{
				acu_clay += (inlet_pre + refer_P) * Pb[i].volume * (porosity_clay1 + (1 - porosity_clay2) * micro_porosity_Clay) * 16 / (compre_1 * 8.314 * Temperature) - (Pb[i].pressure + refer_P) * Pb[i].volume * (porosity_clay1 + (1 - porosity_clay2) * micro_porosity_Clay) * 16 / (compre_2 * 8.314 * Temperature);
			}
			else if (Pb[i].type == 1)
			{
				acu_fracture += (inlet_pre + refer_P) * Pb[i].volume * 16 / (compre_1 * 8.314 * Temperature) - (Pb[i].pressure + refer_P) * Pb[i].volume * 16 / (compre_2 * 8.314 * Temperature);
			}
			else if (Pb[i].type == 2)
			{
				acu_free_OM_HP += (inlet_pre + refer_P) * ((1 - porosity_HP2) * (micro_porosity_HP - n_ad1 / Rho_ad) + porosity_HP1) * Pb[i].volume * 16 / (compre_1 * 8.314 * Temperature) - (Pb[i].pressure + refer_P) * Pb[i].volume * ((1 - porosity_HP2) * (micro_porosity_HP - n_ad2 / Rho_ad) + porosity_HP1) * 16 / (compre_2 * 8.314 * Temperature);
				acu_ad_OM_HP += Pb[i].volume * (1 - porosity_HP2) * (n_ad1 - n_ad2) * 1000;
			}
			else if (Pb[i].type == 3)
			{
				acu_free_OM_LP += (inlet_pre + refer_P) * ((1 - porosity_LP2) * (micro_porosity_LP - n_ad1 / Rho_ad) + porosity_LP1) * Pb[i].volume * 16 / (compre_1 * 8.314 * Temperature) - (Pb[i].pressure + refer_P) * Pb[i].volume * ((1 - porosity_LP2) * (micro_porosity_LP - n_ad2 / Rho_ad) + porosity_LP1) * 16 / (compre_2 * 8.314 * Temperature);
...
void PNMsolver::AMGXsolver_apparent_permeability_REV()
{
	auto start1 = high_resolution_clock::now();
	...
	for (size_t i = 2; i < 52; i++)
	{
		...
		outfile << (macro + micro_advec) * visco(inlet_pre + refer_P, compre(inlet_pre + refer_P), Temperature) * domain_size_cubic * voxel_size / (pow(domain_size_cubic * voxel_size, 2) * (inlet_pre - outlet_pre)) / 1e-21 << "\t"
				<< (inlet_pre + refer_P) / 1e6
				<< endl;
		refer_P = i * 1e6;
		Function_DS(inlet_pre + refer_P);
		initial_condition();
...
void PNMsolver::PressureMatrix_REV()
{
	...
	for (int i = inlet; i < op + inlet; i++)
	{
		if (Pb[i].type == 0)
		{
			B[i - inlet] = -0.016 * (porosity_clay1 + (1 - porosity_clay2) * micro_porosity_Clay) * Pb[i].volume * ((Pb[i].pressure + refer_P) / Pb[i].compre - (Pb[i].pressure_old + refer_P) / Pb[i].compre_old) / (8.314 * Temperature * dt);
		}
		else if (Pb[i].type == 1)
		{
			B[i - inlet] = (-0.016) * Pb[i].volume * ((Pb[i].pressure + refer_P) / Pb[i].compre - (Pb[i].pressure_old + refer_P) / Pb[i].compre_old) / (8.314 * Temperature * dt);
		}
		else if (Pb[i].type == 2)
		{
			B[i - para_macro] = -0.016 * (porosity_HP1 + (1 - porosity_HP2) * micro_porosity_HP) * Pb[i].volume * ((Pb[i].pressure + refer_P) / Pb[i].compre - (Pb[i].pressure_old + refer_P) / Pb[i].compre_old) / (8.314 * Temperature * dt) - (1 - porosity_HP2) * Pb[i].volume / dt * ((1 - 0.016 * (Pb[i].pressure + refer_P) / (Pb[i].compre * 8.314 * Temperature * Rho_ad)) * (n_max_ad * K_langmuir * (Pb[i].pressure + refer_P) / (1 + K_langmuir * (Pb[i].pressure + refer_P))) - (1 - 0.016 * (Pb[i].pressure_old + refer_P) / (Pb[i].compre_old * 8.314 * Temperature * Rho_ad)) * (n_max_ad * K_langmuir * (Pb[i].pressure_old + refer_P) / (1 + K_langmuir * (Pb[i].pressure_old + refer_P))));
		}
		else if (Pb[i].type == 3)
		{
			B[i - para_macro] = -0.016 * (porosity_LP1 + (1 - porosity_LP2) * micro_porosity_LP) * Pb[i].volume * ((Pb[i].pressure + refer_P) / Pb[i].compre - (Pb[i].pressure_old + refer_P) / Pb[i].compre_old) / (8.314 * Temperature * dt) - (1 - porosity_LP2) * Pb[i].volume / dt * ((1 - 0.016 * (Pb[i].pressure + refer_P) / (Pb[i].compre * 8.314 * Temperature * Rho_ad)) * (n_max_ad * K_langmuir * (Pb[i].pressure + refer_P) / (1 + K_langmuir * (Pb[i].pressure + refer_P))) - (1 - 0.016 * (Pb[i].pressure_old + refer_P) / (Pb[i].compre_old * 8.314 * Temperature * Rho_ad)) * (n_max_ad * K_langmuir * (Pb[i].pressure_old + refer_P) / (1 + K_langmuir * (Pb[i].pressure_old + refer_P))));
		}
		...
		for (int j = Pb[i].full_accum - Pb[i].full_coord; j < Pb[i].full_accum; j++)
		{
			if (Tb[j].ID_2 < inlet) // 进口
			{
				if (Pb[Tb[j].ID_1].pressure > Pb[Tb[j].ID_2].pressure)
				{
					B[Tb[j].ID_1 - inlet] += -0.016 * (Pb[Tb[j].ID_1].pressure + refer_P) / (Pb[Tb[j].ID_1].compre * 8.314 * Temperature) * Tb[j].Conductivity * (Pb[Tb[j].ID_1].pressure - Pb[Tb[j].ID_2].pressure);
				}
				else
				{
					B[Tb[j].ID_1 - inlet] += -0.016 * (Pb[Tb[j].ID_2].pressure + refer_P) / (Pb[Tb[j].ID_2].compre * 8.314 * Temperature) * Tb[j].Conductivity * (Pb[Tb[j].ID_1].pressure - Pb[Tb[j].ID_2].pressure);
				}
			}
		...
		{
			a[num + temp - 1] = 0.016 * (porosity_HP1 + (1 - porosity_HP2) * micro_porosity_HP) * Pb[i].volume / (8.314 * Temperature * dt * Pb[i].compre) + (1 - porosity_HP2) * Pb[i].volume / dt * (-2 * n_max_ad * 0.016 * K_langmuir * (Pb[i].pressure + refer_P) - n_max_ad * 0.016 * pow(K_langmuir * (Pb[i].pressure + refer_P), 2) + n_max_ad * K_langmuir * Pb[i].compre * 8.314 * Temperature * Rho_ad) / (Pb[i].compre * 8.314 * Temperature * Rho_ad * pow((1 + K_langmuir * (Pb[i].pressure + refer_P)), 2));
		}
		else if (Pb[i].type == 3)
		{
			a[num + temp - 1] = 0.016 * (porosity_LP1 + (1 - porosity_LP2) * micro_porosity_LP) * Pb[i].volume / (8.314 * Temperature * dt * Pb[i].compre) + (1 - porosity_LP2) * Pb[i].volume / dt * (-2 * n_max_ad * 0.016 * K_langmuir * (Pb[i].pressure + refer_P) - n_max_ad * 0.016 * pow(K_langmuir * (Pb[i].pressure + refer_P), 2) + n_max_ad * K_langmuir * Pb[i].compre * 8.314 * Temperature * Rho_ad) / (Pb[i].compre * 8.314 * Temperature * Rho_ad * pow((1 + K_langmuir * (Pb[i].pressure + refer_P)), 2));
		}
		...
		for (int j = Pb[i].full_accum - Pb[i].full_coord; j < Pb[i].full_accum; j++)
		{
			if (Tb[j].ID_2 < inlet)
			{
				if (Pb[Tb[j].ID_1].pressure > Pb[Tb[j].ID_2].pressure)
				{
					a[ia[i - inlet] + temp - 1] += 0.016 * Tb[j].Conductivity * (2 * (Pb[Tb[j].ID_1].pressure + refer_P) - (Pb[Tb[j].ID_2].pressure + refer_P)) / (8.314 * Temperature * Pb[Tb[j].ID_1].compre); // 对角线
				}
				else
				{
					a[ia[i - inlet] + temp - 1] += 0.016 * Tb[j].Conductivity * (Pb[Tb[j].ID_2].pressure + refer_P) / (8.314 * Temperature * Pb[Tb[j].ID_2].compre); // 对角线
				}
			}
			...
			else
			{
				if (temp1 < temp) // 下三角
				{
					...
					{
						a[ia[i - inlet] + temp - 1] += 0.016 * Tb[j].Conductivity * (2 * (Pb[Tb[j].ID_1].pressure + refer_P) - (Pb[Tb[j].ID_2].pressure + refer_P)) / (8.314 * Temperature * Pb[Tb[j].ID_1].compre); // 对角线
						a[num - 1] += Tb[j].Conductivity * (-0.016 * (Pb[Tb[j].ID_1].pressure + refer_P)) / (8.314 * Temperature * Pb[Tb[j].ID_1].compre);
					}
					else
					{
						a[ia[i - inlet] + temp - 1] += 0.016 * Tb[j].Conductivity * (Pb[Tb[j].ID_2].pressure + refer_P) / (8.314 * Temperature * Pb[Tb[j].ID_2].compre); // 对角线
						a[num - 1] += 0.016 * Tb[j].Conductivity * ((Pb[Tb[j].ID_1].pressure + refer_P) - 2 * (Pb[Tb[j].ID_2].pressure + refer_P)) / (8.314 * Temperature * Pb[Tb[j].ID_2].compre);
					}
				...
				else // 上三角
				{
					...
					{
						a[ia[i - inlet] + temp - 1] += 0.016 * Tb[j].Conductivity * (2 * (Pb[Tb[j].ID_1].pressure + refer_P) - (Pb[Tb[j].ID_2].pressure + refer_P)) / (8.314 * Temperature * Pb[Tb[j].ID_1].compre); // 对角线
						a[num] += Tb[j].Conductivity * (-0.016 * (Pb[Tb[j].ID_1].pressure + refer_P)) / (8.314 * Temperature * Pb[Tb[j].ID_1].compre);
					}
					else
					{

						a[ia[i - inlet] + temp - 1] += 0.016 * Tb[j].Conductivity * (Pb[Tb[j].ID_2].pressure + refer_P) / (8.314 * Temperature * Pb[Tb[j].ID_2].compre); // 对角线
						a[num] += 0.016 * Tb[j].Conductivity * ((Pb[Tb[j].ID_1].pressure + refer_P) - 2 * (Pb[Tb[j].ID_2].pressure + refer_P)) / (8.314 * Temperature * Pb[Tb[j].ID_2].compre);
					}
					if (Tb[j].ID_2 < macro_n)
					{
	...
	for (int i = macro_n + m_inlet; i < pn - m_outlet; i++)
	{
		...
		for (int j = Pb[i].full_accum - Pb[i].full_coord; j < Pb[i].full_accum; j++)
		{
			if (Tb[j].ID_2 >= macro_n && Tb[j].ID_2 < macro_n + m_inlet) // 微孔进出口边界
			{
				if (Pb[Tb[j].ID_1].pressure > Pb[Tb[j].ID_2].pressure)
				{
					B[Tb[j].ID_1 - para_macro] += -0.016 * (Pb[Tb[j].ID_1].pressure + refer_P) / (Pb[Tb[j].ID_1].compre * 8.314 * Temperature) * Tb[j].Conductivity * (Pb[Tb[j].ID_1].pressure - Pb[Tb[j].ID_2].pressure);
				}
				else
				{
					B[Tb[j].ID_1 - para_macro] += -0.016 * (Pb[Tb[j].ID_2].pressure + refer_P) / (Pb[Tb[j].ID_2].compre * 8.314 * Temperature) * Tb[j].Conductivity * (Pb[Tb[j].ID_1].pressure - Pb[Tb[j].ID_2].pressure);
				}
			}
		...
		for (int j = Pb[i].full_accum - Pb[i].full_coord; j < Pb[i].full_accum; j++) // 主对角线的初始值
		{
			if (Tb[j].ID_2 >= macro_n && Tb[j].ID_2 < macro_n + m_inlet)
			{
				if (Pb[Tb[j].ID_1].pressure > Pb[Tb[j].ID_2].pressure)
				{
					a[ia[i - para_macro] + temp - 1] += 0.016 * Tb[j].Conductivity * (2 * (Pb[Tb[j].ID_1].pressure + refer_P) - (Pb[Tb[j].ID_2].pressure + refer_P)) / (8.314 * Temperature * Pb[Tb[j].ID_1].compre); // 对角线
				}
				else
				{
					a[ia[i - para_macro] + temp - 1] += 0.016 * Tb[j].Conductivity * (Pb[Tb[j].ID_2].pressure + refer_P) / (8.314 * Temperature * Pb[Tb[j].ID_2].compre); // 对角线
				}
			}
			else if (Tb[j].ID_2 >= pn - m_outlet)
			{
...
void PNMsolver::PressureMatrix_Per_REV()
{
	int num;	  // 每行第一个非0参数的累计编号
	int num1 = 0; // 矩阵中每行的非0数据数量
	int temp;	  // 确定对角线前面的数据数量
		...
		for (int j = Pb[i].full_accum - Pb[i].full_coord; j < Pb[i].full_accum; j++) // 主对角线的初始值
		{
			...
			else
			{
				if (temp1 < temp) // 下三角
				{
					if (Pb[Tb[j].ID_1].pressure > Pb[Tb[j].ID_2].pressure)
					{
						a[ia[i - para_macro] + temp - 1] += 0.016 * Tb[j].Conductivity * (2 * (Pb[Tb[j].ID_1].pressure + refer_P) - (Pb[Tb[j].ID_2].pressure + refer_P)) / (8.314 * Temperature * Pb[Tb[j].ID_1].compre) + Tb[j].Surface_diff_conduc * K_langmuir / pow(1 + K_langmuir * (Pb[Tb[j].ID_1].pressure + refer_P), 2); // 对角线
						a[num - 1] += 0.016 * Tb[j].Conductivity * (-(Pb[Tb[j].ID_1].pressure + refer_P)) / (8.314 * Temperature * Pb[Tb[j].ID_1].compre) + Tb[j].Surface_diff_conduc * (-K_langmuir) / pow(1 + K_langmuir * (Pb[Tb[j].ID_2].pressure + refer_P), 2);
					}
					else
					{
						a[ia[i - para_macro] + temp - 1] += 0.016 * Tb[j].Conductivity * (Pb[Tb[j].ID_2].pressure + refer_P) / (8.314 * Temperature * Pb[Tb[j].ID_2].compre) + Tb[j].Surface_diff_conduc * K_langmuir / pow(1 + K_langmuir * (Pb[Tb[j].ID_1].pressure + refer_P), 2); // 对角线
						a[num - 1] += 0.016 * Tb[j].Conductivity * ((Pb[Tb[j].ID_1].pressure + refer_P) - 2 * (Pb[Tb[j].ID_2].pressure + refer_P)) / (8.314 * Temperature * Pb[Tb[j].ID_2].compre) + Tb[j].Surface_diff_conduc * (-K_langmuir) / pow(1 + K_langmuir * (Pb[Tb[j].ID_2].pressure + refer_P), 2);
					}
				...
				else // 上三角
				{
					if (Pb[Tb[j].ID_1].pressure > Pb[Tb[j].ID_2].pressure)
					{
						...
						a[ia[i - para_macro] + temp - 1] += 0.016 * Tb[j].Conductivity * (2 * (Pb[Tb[j].ID_1].pressure + refer_P) - (Pb[Tb[j].ID_2].pressure + refer_P)) / (8.314 * Temperature * Pb[Tb[j].ID_1].compre) + Tb[j].Surface_diff_conduc * K_langmuir / pow(1 + K_langmuir * (Pb[Tb[j].ID_1].pressure + refer_P), 2); // 对角线
						a[num] += 0.016 * Tb[j].Conductivity * (-(Pb[Tb[j].ID_1].pressure + refer_P)) / (8.314 * Temperature * Pb[Tb[j].ID_1].compre) + Tb[j].Surface_diff_conduc * (-K_langmuir) / pow(1 + K_langmuir * (Pb[Tb[j].ID_2].pressure + refer_P), 2);
					}
					else
					{
						a[ia[i - para_macro] + temp - 1] += 0.016 * Tb[j].Conductivity * (Pb[Tb[j].ID_2].pressure + refer_P) / (8.314 * Temperature * Pb[Tb[j].ID_2].compre) + Tb[j].Surface_diff_conduc * K_langmuir / pow(1 + K_langmuir * (Pb[Tb[j].ID_1].pressure + refer_P), 2); // 对角线
						a[num] += 0.016 * Tb[j].Conductivity * ((Pb[Tb[j].ID_1].pressure + refer_P) - 2 * (Pb[Tb[j].ID_2].pressure + refer_P)) / (8.314 * Temperature * Pb[Tb[j].ID_2].compre) + Tb[j].Surface_diff_conduc * (-K_langmuir) / pow(1 + K_langmuir * (Pb[Tb[j].ID_2].pressure + refer_P), 2);
					}
...
void PNMsolver::Eigen_solver_per(double i)
{
	...
	int inter_n{0};										  // The interation of outer loop of Newton-raphoon method
	double total_flow = 0;								  // accumulation production
	ofstream outfile("Intrinsic_Permeability_Eigen.txt"); // output permeability;
	Flag_eigen = true;
	...
	outfile << (macro + micro_advec + micro_diff) * visco(inlet_pre, compre(inlet_pre), Temperature) * domain_size_cubic * voxel_size / (pow(domain_size_cubic * voxel_size, 2) * (inlet_pre - outlet_pre)) << "\t"
			<< inlet_pre / 1e6
			<< endl;
	// output(1, 1);
	outfile.close();
...
void PNMsolver::Paramentinput()
{
	...
	for (const auto &file : fileList)
	{
		if (file.find(string("full_pore")) != string::npos)
		{
			...
			{
				double waste{0};
				// porefile >> Pb[i].X >> Pb[i].Y >> Pb[i].Z >> waste >> Pb[i].Radiu >> Pb[i].Half_coord >> Pb[i].half_accum >> Pb[i].type >> Pb[i].full_coord >> Pb[i].full_accum; //FIB-SEM
...
void PNMsolver::para_cal()
{
	...
	for (int i = 0; i < 2 * tn; i++)
	{
		...
		else if ((Tb_in[i].ID_1 < macro_n && Tb_in[i].ID_2 >= macro_n))
		{
			...
			temp2 = abs(Slip * ko * Tb_in[i].Radiu * angle2 / (Average_visco * length2));
		...
		else if ((Tb_in[i].ID_1 >= macro_n && Tb_in[i].ID_2 < macro_n))
		{
			...
			temp1 = abs(Slip * ko * Tb_in[i].Radiu * angle1 / (Average_visco * length1));
...
void PNMsolver::Eigen_subroutine_per(Eigen::SparseMatrix<double, Eigen::RowMajor> &A0, Eigen::VectorXd &B0)
{
	...
	auto stop = high_resolution_clock::now();
	auto duration = duration_cast<milliseconds>(stop - start);
	cout << "Eigen subroutine Time-consuming = " << duration.count() << " MS" << endl;
	// endTime = clock();
	// cout<<"solver time:"<<(endTime-startTime)/1000<<endl;
}
...
double PNMsolver::micro_outlet_advec_Q()
{
	...
	for (int i = pn - m_outlet; i < pn; i++)
	{
		...
		{
			Q_outlet += (Pb[Tb[j].ID_1].pressure - Pb[Tb[j].ID_2].pressure) * Tb[j].Conductivity; // 体积流量
																								  // out_flux << Pb[Tb[j].ID_1].pressure << "  " << Pb[Tb[j].ID_2].pressure << "  " << abs(Q_outlet) << "  " << 0 << "  " << i << endl;
		}
	}
	return abs(Q_outlet);
}; // 出口微孔流量
...
void PNMsolver::output(int n, bool m)
{
	...
	// ofstream outfile2("conduct3.txt");
	// for (size_t i = 0; i < 2 * tn; i++)
	// {
	// 	outfile2 << Tb_in[i].Conductivity << " ; " << Tb[i].Conductivity << endl;
	// }
	// outfile2.close();
}
...
void PNMsolver::AMGXsolver()
{
	...
	AMGX_pin_memory(a, nnz_amgx * sizeof(double));
	AMGX_pin_memory(B, sizeof(double) * n_amgx);
	// AMGX_pin_memory(X, sizeof(double) * n_amgx);
	AMGX_pin_memory(dX, sizeof(double) * n_amgx);
	// AMGX_pin_memory(Xn, sizeof(double) * n_amgx);
	...
	do
	{
		...
		// if (total_flow / total_p > 0.9+0.01 * nn)
		// {
		// 	output(time_step);
		// 	nn++;
		// }
	...
	for (int i = inlet; i < op + inlet; i++)
	{
		...
		for (int j = Pb[i].full_accum - Pb[i].full_coord; j < Pb[i].full_accum; j++)
		{
			if (Tb[j].ID_2 < inlet) // 进口
			{
				if (Pb[Tb[j].ID_1].pressure > Pb[Tb[j].ID_2].pressure)
				{
					B[Tb[j].ID_1 - inlet] += -Tb[j].Conductivity * (Pb[Tb[j].ID_1].pressure - Pb[Tb[j].ID_2].pressure);
				}
				else
				{
					B[Tb[j].ID_1 - inlet] += -Tb[j].Conductivity * (Pb[Tb[j].ID_1].pressure - Pb[Tb[j].ID_2].pressure);
	...
	for (int i = macro_n + m_inlet; i < pn - m_outlet; i++)
	{
		...
		for (int j = Pb[i].full_accum - Pb[i].full_coord; j < Pb[i].full_accum; j++)
		{
			if (Tb[j].ID_2 >= macro_n && Tb[j].ID_2 < macro_n + m_inlet) // 微孔进出口边界
			{
				if (Pb[Tb[j].ID_1].pressure > Pb[Tb[j].ID_2].pressure)
				{
					B[Tb[j].ID_1 - para_macro] += -Tb[j].Conductivity * (Pb[Tb[j].ID_1].pressure - Pb[Tb[j].ID_2].pressure);
				}
				else
				{
					B[Tb[j].ID_1 - para_macro] += -Tb[j].Conductivity * (Pb[Tb[j].ID_1].pressure - Pb[Tb[j].ID_2].pressure);
...
int main(int argc, char **argv)
{
	...
	// Berea.AMGXsolver_apparent_permeability_REV();
	// Berea.Intrinsic_permeability(myid);
	...
	// 	nad << "pressure=" << pressure << ";gas=" << (porosity - out / Rho_ad) * rho << ";porosity=" <outlet_pre< (porosity - out / Rho_ad) << ";rho = " << rho << endl;
	...
	// 	double Knusen_number_ID1{0};FGMRES_AGGREGATION
	// 	Knusen_number_ID1 = vis / (pressure)*sqrt(pi * z * 8.314 * Temperature / (2 * 0.016)) / (L_OM_1);
	// 	double Slip_ID1 = Berea.Function_Slip(Knusen_number_ID1);
	// 	double Apparent_K_ID1 = Slip_ID1 * K_OM_HP + vis * Ds * n_max_ad * K_langmuir / pow(1 + K_langmuir * (pressure), 2) / rho;
	// 	ka << Apparent_K_ID1 / 1e-21 << ";" << pressure / 1e6 << endl;
	// }
	// ka.close();

	// ofstream ka2("ka_OM2.txt");
	// for (size_t i = 1; i < 51; i++)
	// {
	// 	auto pressure = i * 1e6;
	// 	auto z{Berea.compre(pressure)};
	...
	// 	Berea.Function_DS(pressure);
	// 	double Knusen_number_ID1{0};
	// 	Knusen_number_ID1 = vis / (pressure)*sqrt(pi * z * 8.314 * Temperature / (2 * 0.016)) / (L_OM_2);
	// 	double Slip_ID1 = Berea.Function_Slip(Knusen_number_ID1);
	// 	double Apparent_K_ID2 = Slip_ID1 * K_OM_LP + vis * Ds * n_max_ad * K_langmuir / pow(1 + K_langmuir * (pressure), 2) / rho;
	// 	ka2 << Apparent_K_ID2 / 1e-21 << ";" << pressure / 1e6 << endl;
	// }
	// ka2.close();

	// ofstream ka3("ka_clay.txt");
	// for (size_t i = 1; i < 51; i++)
	// {
	// 	auto pressure = i * 1e6;
	// 	auto z{Berea.compre(pressure)};
	...
	// 	Berea.Function_DS(pressure);
	// 	double Knusen_number_ID1{0};
	// 	Knusen_number_ID1 = vis / (pressure)*sqrt(pi * z * 8.314 * Temperature / (2 * 0.016)) / (L_CLAY);
	// 	double Slip_ID1 = Berea.Function_Slip_clay(Knusen_number_ID1);
	// 	double Apparent_K_ID2 = Slip_ID1 * K_OM_LP;
	// 	ka3 << Apparent_K_ID2 / 1e-21 << ";" << pressure / 1e6 << endl;
	// }
	// ka3.close();
